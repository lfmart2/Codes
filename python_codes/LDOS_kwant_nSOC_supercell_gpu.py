    ...
    with h5py.File(file_path, "r") as file:
        ...
        for label in list_Ri:
            ...
            if has_dst:
                ...
                final_block[xp.ix_(cols, rows)] += values_conj.T
    return _to_numpy(final_block)
...
if __name__ == "__main__":
    ...
    if "--quick-test" in args:
        if len(args) < 2:
            raise SystemExit(
                "Usage: python DOS_kwant.py --quick-test <wannier90_hr_up.dat> [--use-gpu]"
            )
        hr = load_hr(args[1])
    ...
    if len(args) < 4:
        raise SystemExit(
            ...
            "<wannier90_hr_dn.dat> <output_dn.csv> [--use-gpu]"
        )
    hr = load_hr(args[0])
