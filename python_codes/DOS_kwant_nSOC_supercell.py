...
if __name__ == "__main__":
    ...
    print(f"Minimum bound energy for spin-up: {emax_up}")
    energies_up = np.linspace(emin_up + 0.01, Emax_req_up, 800)
    dos_up = rho(energies_up)
    filename_up = sys.argv[3]
    df = pd.DataFrame({'Energies': energies_up, 'DOS': dos_up})
    ...
    print(f"Spin down System:")
    report_system_size(fsys)
    rho = kwant.kpm.SpectralDensity(fsys)
    emin_dn, emax_dn = rho.bounds
    Emax_req_dn = E_F + 1.0
    print(f"Minimum bound energy for spin-down: {emin_dn}")
    print(f"Minimum bound energy for spin-down: {emax_dn}")
    energies_dn = np.linspace(emin_dn + 0.01, Emax_req_dn, 800)
    dos_dn = rho(energies_dn)
    filename_dn = sys.argv[4]
    df = pd.DataFrame({'Energies': energies_dn, 'DOS': dos_dn})
