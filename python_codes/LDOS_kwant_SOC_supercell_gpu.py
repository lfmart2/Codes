...
def _get_array_module(use_gpu: bool):
    if use_gpu:
        if cp is None:
            raise RuntimeError(
                "CuPy is not available. Install cupy or run without --use-gpu."
            )
        return cp
    return np
...
def coupling_hydrogen_slab(
    ...
    *,
    ...
    if not has_src and not has_dst:
        return _to_numpy(final_block)

    # Distances for polynomial coupling (separate spin channels).
    dist_up = (
        xp.array([mol_distance[src_idx]], dtype=float) if has_src else xp.array([])
    )
    dist_dn = (
        xp.array([mol_distance[src_idx + 1]], dtype=float)
        if has_src
        else xp.array([])
    )
    dist_up_conj = (
        xp.array([mol_distance[dst_idx]], dtype=float) if has_dst else xp.array([])
    )
    dist_dn_conj = (
        xp.array([mol_distance[dst_idx + 1]], dtype=float)
        if has_dst
        else xp.array([])
    )
    ...
    with h5py.File(file_path, "r") as file:
        ...
        for label in list_Ri:
            ...
            data_re = xp.asarray(block_interaction[label])
            data_im = xp.asarray(block_interaction[f"{label}_im"])
            cols = xp.asarray(interaction_indices + hydrogen_orbitals)
            ...
            if has_src:
                ...
                final_block[xp.ix_(rows, cols)] += values
            ...
            if has_dst:
                ...
                final_block[xp.ix_(cols, rows_conj)] += values_conj.T
    return _to_numpy(final_block)
...
if __name__ == "__main__":
    ...
    args = [arg for arg in sys.argv[1:] if arg != "--use-gpu"]
    # Expect Wannier90 Hamiltonian path and output filename from CLI.
    if "--quick-test" in args:
        if len(args) < 2:
            raise SystemExit(
                "Usage: python DOS_kwant.py --quick-test <wannier90_hr.dat> [--use-gpu]"
            )
        hr = load_hr(args[1])
    ...
    if len(args) < 2:
        raise SystemExit(
            "Usage: python DOS_kwant.py <wannier90_hr.dat> <output.csv> [--use-gpu]"
        )
    hr = load_hr(args[0])
