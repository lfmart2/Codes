...
%         wlp22(ii) = wlp1(ii)*(dot(Vs(:,2),Vs1(:,2))+dot(Vs(:,2),Vs1(:,1)));
    ...
    % Compute Wilson loop products for each permutation block
...
%% plots
% Plot tiled copies of two spectra shifted in x and y to show periodicity
figure; hold on;
...
% Loop over ky (here single value) and kx grid to compute overlaps
for ii = 1:length(kky)
    for i = 2:length(kkx)-1
        ...
        Ds1 = D1(ind,ind);
        Vs1 = V1(:,ind);
        % Compute overlap between corresponding eigenvectors at adjacent kx
...
%% Assignment 2
clear variables
% Create 9x9 and 6x6 identity-like shift matrices for Kronecker construction
t0=zeros(9,9);
T0=zeros(6,6);
for i=1:length(T0)-1
    T0(i,i+1)=1;
...
T0(length(T0),1)=1;
% Base Hamiltonian built from Kronecker of 9x9 and symmetric 6x6 blocks
h0=kron(t0,T0+T0');
t=linspace(0,1.5,7);
Th1=zeros(6,6);
Th2=zeros(6,6);
...
for i=1:length(t)
    ...
    % Time-dependent perturbation added with Hermitian conjugate
...
[X,Y]=meshgrid(x,y);
Z=0;
% Sum Gaussian peaks weighted by eigenvector differences to form surface Z
...
clear variables
% Define reciprocal-space high-symmetry points (Gamma, K, K', M)
Gamma = [0, 0];
K = [2*pi/3 0];
KK = [pi/3 sqrt(3)*pi/3];
M = [pi/2 pi/(2*sqrt(3))];
...
alpha2 = [1 sqrt(3)];
alpha3 = [1 -sqrt(3)];
% Hamiltonian at Gamma (tight-binding with complex phase factors)
...
[V,D]=eig(h2);
% Compute overlap matrix hmd between first two eigenvectors via symmetry R3
for j=1:2
    for jj=1:2
